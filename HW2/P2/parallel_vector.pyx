...
cpdef move_data_fine_grained(np.int32_t[:] counts,
   ...
   with nogil, parallel(num_threads=4):
       for r in range(repeat):
           ...
           # Case 1: when source and destination locations are different 
               if( src[idx] != dest[idx] ):
                   if( src[idx] < dest[idx] ):  # ensure ordering to prevent deadlock
           ...
           # Case 2: when source and destination locations are the same; only needs a single lock
               else:
                   acquire( &locks[src[idx]] )
                   if counts[src[idx]] > 0:
                       counts[dest[idx]] += 1
       ...
       for r in range(repeat):
           ...
           # Case 1: when source and destination locations are different
               if( src[idx]/N != dest[idx]/N ):   
                   if( src[idx]/N < dest[idx]/N ):   # ensure ordering to prevent deadlock
