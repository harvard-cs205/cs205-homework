...
#Worked with Sami Goche on these sections
#discussing / white-boarding some ideas. 
...
cpdef move_data_fine_grained(np.int32_t[:] counts,
  ...
  with nogil:
      for r in range(repeat):
          ...
          # Need to enforce some order to how/when we acquire a lock (for justification why look 
          # at the Dining Philosophers problem) so we just arbitrarily chose that the source
          # must be the larger of the two. Acquire the locks and release in the same order, and when
          # the source and dest are equal, we only acquire the lock for one of them.
...
cpdef move_data_medium_grained(np.int32_t[:] counts,
  ...
  with nogil:
      for r in range(repeat):
        #similar idea to above, we are going to parallelize and acquire locks
        #by enforcing some order, only now we have an "N" value so we bucket
        #which locks we actually acquire. 
          for idx in prange(src.shape[0], num_threads=4, schedule=dynamic):
              bothEqual = 0
