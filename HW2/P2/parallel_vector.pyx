    ...
    with nogil:
        for r in range(repeat):
            for idx in prange(src.shape[0], num_threads=4):
                max_id = src[idx] if src[idx] > dest[idx] else dest[idx]
                min_id = src[idx] if src[idx] < dest[idx] else dest[idx]
                acquire(&(locks[max_id]))
                if min_id < max_id:
                    acquire(&(locks[min_id]))
                if counts[src[idx]] > 0:
                    counts[src[idx]] -= 1
                    counts[dest[idx]] += 1
                if min_id < max_id:
                    release(&(locks[min_id]))
                release(&(locks[max_id]))
...
#just want to see how long it takes to lock twice and unlock
cpdef time_locks_fine_grained(np.int32_t[:] counts,
                 np.int32_t[:] src,
                 np.int32_t[:] dest,
                 int repeat):
    cdef:
...
cpdef time_locks_medium_grained(np.int32_t[:] counts,
                                np.int32_t[:] src,
                                np.int32_t[:] dest,
                                int repeat,
                                int N):
