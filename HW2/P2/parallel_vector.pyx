   ...
   with nogil:
       for r in range(repeat):
           for idx in prange(src.shape[0], num_threads=4):
              ...
              # Need to make sure locks are acquired in order to prevent contention
              if src[idx] > dest[idx]: 
                  maxidx = src[idx]
                  minidx = dest[idx]
              else: # This happens if src[idx] < dest[idx], also if they are equal
                  maxidx = dest[idx]
                  minidx = src[idx]

              acquire(&locks[minidx])

              if minidx != maxidx: acquire(&locks[maxidx])

              if counts[src[idx]] > 0:
                  counts[dest[idx]] += 1
                  counts[src[idx]] -= 1

              if minidx != maxidx: release(&locks[maxidx])
              
              release(&locks[minidx])
   ...
   with nogil:
       for r in range(repeat):
           for idx in prange(src.shape[0], num_threads=4):
               ...
               # Need to make sure locks are acquired in order to prevent contention
               if src[idx] > dest[idx]: 
                   # Now I do an integer division by N to create blocks for the locks
                   maxidx = src[idx]/N
                   minidx = dest[idx]/N
               else: # This happens if src[idx] < dest[idx], also if == !!!!
                   maxidx = dest[idx]/N
                   minidx = src[idx]/N

               acquire(&(locks[maxidx]))

               if maxidx != minidx: acquire(&(locks[minidx]))
               
               if counts[src[idx]] > 0:
                   counts[dest[idx]] += 1
                   counts[src[idx]] -= 1

               if maxidx != minidx: release(&(locks[minidx]))

               release(&(locks[maxidx]))
