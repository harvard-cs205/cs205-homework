...
cpdef move_data_fine_grained(np.int32_t[:] counts,
   ...
   for r in range(repeat):
       for idx in prange(src.shape[0], nogil = True , num_threads = 4 ):
           sidx, didx = src[idx], dest[idx]
           if sidx == didx:
               acquire(&(locks[didx]))
           else:
               acquire(&(locks[min(didx,sidx)]))
               acquire(&(locks[max(didx,sidx)]))
           if counts[src[idx]] > 0:
               counts[dest[idx]] += 1
               counts[src[idx]] -= 1
           if sidx == didx:
               ...
               release(&(locks[min(didx,sidx)]))
               release(&(locks[max(didx,sidx)]))
...
cpdef move_data_medium_grained(np.int32_t[:] counts,
   ...
   for r in range(repeat):
       for idx in prange(src.shape[0], nogil = True , num_threads = 4 ):
           sidx, didx = src[idx]/N, dest[idx]/N
           if sidx == didx:
               acquire(&(locks[didx]))
           else:
               acquire(&(locks[min(didx,sidx)]))
