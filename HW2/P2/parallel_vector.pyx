...
cpdef move_data_fine_grained(np.int32_t[:] counts,
    ...
    for r in range(repeat):
        for idx in prange(src.shape[0], nogil=True, num_threads=4, schedule=dynamic):
            ...
            # otherwise lock smaller one first
            ...
            # same logic for releasing as for aquiring
            if src[idx] == dest[idx]:
...
cpdef move_data_medium_grained(np.int32_t[:] counts,
    ...
    for r in range(repeat):
        for idx in prange(src.shape[0], nogil=True, num_threads=4, schedule=dynamic):
            # precalculate lock vals; have consecutive N entries
            srcLock = src[idx]/N
            destLock = dest[idx]/N
            # if same block, only need one lock
            if srcLock == destLock:
                acquire(&locks[srcLock])
            # otherwise lock smaller one first
            elif srcLock < destLock:
                acquire(&locks[srcLock])
                acquire(&locks[destLock])
            else:
                acquire(&locks[destLock])
                acquire(&locks[srcLock])
            ...
            # same logic for releasing as for aquiring
            if srcLock == destLock:
                release(&locks[srcLock])
            elif srcLock < destLock:
                release(&locks[srcLock])
                release(&locks[destLock])
            else:
                release(&locks[destLock])
                release(&locks[srcLock])
