   ...
   with nogil:
       for r in range(repeat):
           for idx in prange(src.shape[0], num_threads=4):
               # acquire locks, every element has its own lock
               # to avoid dead-locking, 
               # the locking order must be the same between different threads
               if src[idx] > dest[idx]:
               ...
               # release locks in the opposite order, to avoid dead-locking
               if src[idx] > dest[idx]:
...
cpdef move_data_medium_grained(np.int32_t[:] counts,
   ...
   with nogil:
       for r in range(repeat):
           for idx in prange(src.shape[0], num_threads=4):
               # acquire locks, every N element share a lock
               # to avoid dead-locking, 
               # the locking order must be the same between different threads
