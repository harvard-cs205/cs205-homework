...
cpdef move_data_medium_grained(np.int32_t[:] counts,
   ...
   with nogil:
       for r in range(repeat):
           for idx in prange(src.shape[0], num_threads = 4):
               if counts[src[idx]] > 0:
                   Nlocks_src = (src[idx] + N - 1) / N
                   Nlocks_dest = (dest[idx] + N - 1) / N
                   if Nlocks_src == Nlocks_dest:
                       acquire(&locks[Nlocks_dest])
                       counts[dest[idx]] += 1
                       counts[src[idx]] -= 1
                       release(&locks[Nlocks_dest])
                   else:
                      acquire(&locks[Nlocks_dest])
                      counts[dest[idx]] += 1
                      release(&locks[Nlocks_dest])
                      acquire(&locks[Nlocks_src])
                      counts[src[idx]] -= 1
                      release(&locks[Nlocks_src])   
