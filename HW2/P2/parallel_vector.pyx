...
cpdef move_data_fine_grained(np.int32_t[:] counts,
    ...
    for r in range(repeat):
        for idx in prange(src.shape[0], nogil=True, num_threads=4, schedule=dynamic):
            # Acquire lock for both corresponding src[idx] and dest[idx] element in counts in same order
            if src[idx] == dest[idx]:
                continue
            if src[idx] < dest[idx]: 
            ...
            # Release locks in same order as before
            if src[idx] < dest[idx]:
