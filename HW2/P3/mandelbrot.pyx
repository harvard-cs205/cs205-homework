...
cpdef mandelbrot(np.complex64_t [:, :] in_coords,
    ...
    cdef:
       int i, j, k, signs_sum
       AVX.float8 four, two, one, z_real, z_imag, c_real, c_imag, iters, max_iters8, z_real_sq, z_imag_sq, mag_sq, greater_than_4_mask, increment, incremented_last_time
       float [:, :] in_coords_real, in_coords_imag, real_section, imag_section
       float mag_sq_temp[8], zreal[8], zimag[8], check_iters[8]
    ...
    with nogil:
        ...
        neg_nan = AVX.greater_than(four, two)
        for i in prange(in_coords.shape[0], schedule='static', chunksize=1, num_threads=threads):
            for j in range(0, in_coords.shape[1], 8):
                ...
                while AVX.signs(AVX.less_than(iters, max_iters8)) == 255:
                    ...
                    """AVX.to_mem(iters, &check_iters[0])
                    AVX.to_mem(mag_sq, &mag_sq_temp[0])
                    with gil:
                        print 'iters' + str(check_iters)
                        print 'mag' + str(mag_sq_temp)"""
                    greater_than_4_mask = AVX.greater_than(mag_sq, four)
                    #Returns true when all values > 4
                    signs_sum = AVX.signs(greater_than_4_mask)
                    if signs_sum == 255:
                        break
                    ...
                    """AVX.to_mem(z_real, &zreal[0])
                    AVX.to_mem(z_imag, &zimag[0])
                    with gil:
                        print 'z_real' + str(zreal)
                        print 'z_imag' + str(zimag)"""
                    #Increment only those always and currently <= 4
                    increment = AVX.bitwise_and(AVX.bitwise_andnot(greater_than_4_mask, incremented_last_time), one)
                    iters = AVX.add(iters, increment)
                    incremented_last_time = AVX.bitwise_andnot(greater_than_4_mask, neg_nan)
