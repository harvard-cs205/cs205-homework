...
cdef void sub_update(FLOAT[:, ::1] XY,
    ...
    cdef:
        ...
        int j, dim, x, y
        float eps = 1e-5
        unsigned int grid_x, grid_y, grid_size
    ...
    #We check all the points in the grid that is 2 spaces away from the ball
    for x in range(grid_x + 1, grid_x + 3):
        for y in range(grid_y + 1, grid_y + 4 + grid_x - x):
            #check if x and y are bound within the [0,1]^2 space, and check if the grid value is smaller than count
            if x < grid_size and y < grid_size and Grid[x,y] < count:
                XY2 = &(XY[Grid[x,y], 0])
                V2 = &(V[Grid[x,y], 0])
                if overlapping(XY1, XY2, R):
                    # SUBPROBLEM 4: Add locking

                    #Acquire lock for XY2
                    acquire(&(locks[Grid[x,y]]))
...
cpdef update(FLOAT[:, ::1] XY,
    ...
    with nogil:
        ...
        # scheduling.
        for i in prange(count, schedule = 'static', num_threads = 4, chunksize = count/4):
            for dim in range(2):
        ...
        for i in prange(count, schedule = 'static', num_threads = 4, chunksize = count/4):
            # We first check whether the ball is inside the grid
            if XY[i,0] >= 0 and XY[i,0] <= 0 and XY[i,1] >= 0 and XY[i,1] <= 0:
                grid_x = <unsigned int>(XY[i,0] / grid_spacing)
                grid_y = <unsigned int>(XY[i,1] / grid_spacing)
