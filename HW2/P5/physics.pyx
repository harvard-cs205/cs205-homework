...
cdef void sub_update(FLOAT[:, ::1] XY,
    ...
    cdef:
        FLOAT *XY1, *XY2, *V1, *V2,
        int dim, X1_idx, Y1_idx, idx_x, idx_y
        float eps = 1e-5

    XY1 = &(XY[i, 0])
    V1 = &(V[i, 0])

    X1_idx = int(XY1[0]/grid_spacing)
    Y1_idx = int(XY1[1]/grid_spacing)
    #############################################################
    ...
    # check for colliding balls in a 5x5 grid
    #acquire(&(locks[Grid[idx_x, idx_y]]))
    for idx_x in range(X1_idx - 2, X1_idx + 3):
        for idx_y in range(Y1_idx - 2, Y1_idx + 3):
            if idx_x < Grid.shape[0] and idx_y < Grid.shape[0] and Grid[idx_x, idx_y] < count and Grid[idx_x, idx_y] != i:
                    # lock indices
                    acquire(&(locks[Grid[idx_x, idx_y]]))
                    XY2 = &(XY[Grid[idx_x, idx_y],0])
                    V2 = &(V[Grid[idx_x, idx_y],0])
                    ...
                    # release locks
                    release(&(locks[Grid[idx_x, idx_y]]))
