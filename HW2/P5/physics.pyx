...
cdef void sub_update(FLOAT[:, ::1] XY,
    ...
    gi=<int> (XY1[0] / grid_spacing)
    gj=<int> (XY1[1] / grid_spacing)
    grid_size=<int> (1.0/grid_spacing+1)

    xl=max(gi-2, 0)
    xu=min(gi+3, grid_size)
    yl=max(gj-2, 0)
    yu=min(gj+3, grid_size)

    #for j in range(i + 1, count):
    for gii in range(xl, xu):
        for gjj in range(yl, yu):
            j=Grid[gii, gjj]
            if j>i and j< count:
                acquire(&locks[j])
                XY2 = &(XY[j, 0])
...
cpdef update(FLOAT[:, ::1] XY,
    ...
    with nogil:
        ...
        for i in prange(count, schedule='static', chunksize=2500, num_threads=1):
            for dim in range(2):
