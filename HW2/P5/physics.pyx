...
cdef void sub_update(FLOAT[:, ::1] XY,
    ...
    gridx, gridy = int(XY1[0]/grid_spacing), int(XY1[1]/grid_spacing)
    ## Check the nearby three grids.
    for x in xrange(max(0, gridx-3), min(Grid.shape[0], gridx+4)):
        for y in xrange(max(0, gridy-3), min(Grid.shape[0], gridy+4)):
            j = Grid[x, y]
            # Make sure we do not detect double collision
            if j<=i: continue
...
cpdef update(FLOAT[:, ::1] XY,
    ...
    with nogil:
        ...
        for i in prange(count, schedule = 'static', chunksize = count/4, num_threads = n_threads):
            for dim in range(2):
        ...
        for i in prange(count, schedule = 'static', chunksize = count/4, num_threads = n_threads):
            for dim in range(2):
                XY[i, dim] += V[i, dim] * t
                XY[i, dim] = min(max(XY[i, dim], 0), 1)
            gridx, gridy = int(XY[i, 0]/grid_spacing), int(XY[i, 1]/grid_spacing)
            Grid[gridx, gridy] = i
