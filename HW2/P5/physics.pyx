...
cdef void sub_update(FLOAT[:, ::1] XY,
    ...
    # SUBPROBLEM 2:
    # Finding the grid ceil of the ball i
    ind_x = <int> (XY1[0] / grid_spacing)
    ind_y = <int> (XY1[1] / grid_spacing)
    for sx in range(5):
        for sy in range(5):  # Checking in the 5*5 square center on i
            if ((ind_x + (sx - 2) >= 0) and (ind_x + (sx - 2) < Grid.shape[0]) and
                (ind_y + (sy - 2) >= 0) and (ind_y + (sy - 2) < Grid.shape[0])):
                    j = Grid[ind_x + (sx - 2), ind_y + (sy - 2)]
                    if j > i:  # Not to do twice the same collision
                        XY2 = &(XY[j, 0])
                        V2 = &(V[j, 0])
                        if overlapping(XY1, XY2, R):
                            # SUBPROBLEM 4:
...
cpdef update(FLOAT[:, ::1] XY,
    ...
    with nogil, parallel(num_threads=num_threads):
        ...
        # scheduling.
        for i in prange(count, schedule='static', chunksize=count / 4):
            for dim in range(2):
        ...
        for i in prange(count, schedule='static', chunksize=count / 4):
            for dim in range(2):
                XY[i, dim] += V[i, dim] * t
            # SUBPROBLEM 2:Updating the grid values
