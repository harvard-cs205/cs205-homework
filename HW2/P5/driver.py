...
if __name__ == '__main__':
    ...
    while True:
        ...
        if total_time > anim_step:
            ...
            # numbering positions : (1, [x1, y1]), (2, [x2, y2]), ...
            index_and_position = zip(range(len(positions)), (positions / grid_spacing).astype(int))
            # sort using Z order curve
            index_and_position.sort(cmp_zorder)
            # get new order
            new_order = map(lambda (i, position): i, index_and_position)
            # reset grid
            grid[:, :] = -1
            
            # reorder the existing position, velocity 
            positions = np.array([positions[i] for i in new_order])
            velocities = np.array([velocities[i] for i in new_order])
