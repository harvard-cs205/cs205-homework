...
# only letters which are capitalized and ends with a period. Be sure that filtering doesn't change ordering of words!
def filterWords(word):
	return not (word.isupper() or (word[:-1].isupper() and word[-1] == '.') or word.isdigit())
...
rdd = threeWordPhrases.map(lambda (idx, ((w1, w2), w3)): ((w1,w2,w3), 1)) \
	# Count total number of each distinct three word phrase
	.reduceByKey(lambda count1,count2: count1+count2) \
	# Group by first two words in three word phrase being the key
...
# Print 10 generated phrases of max 20 words each
for i in xrange(10):
