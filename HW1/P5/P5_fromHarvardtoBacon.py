...
def find_distance(neighbors,
    ...
    # First step, find Harvard_University and its direct neighbors
    pages_rdd = neighbors.filter(lambda (k,v): k == Harvard_University)

    # Check we have the good one
    assert pages_rdd.collect()[0][0] == Harvard_University
    ...
    for i in range(iterations): 
        ...
        # Here we check if we have found Kevin_Bacon
        if direct_neighbors.filter( lambda (k,v) : k == Kevin_Bacon  ).count() > 0:
...
if __name__ == '__main__':
	...
	distance = find_distance(neighbors,page_names,iterations=10 ) 
