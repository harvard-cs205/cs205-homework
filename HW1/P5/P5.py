...
links = links.map(lambda x: x.split(': ')).map(lambda x: (x[0],x[1])).mapValues(lambda x: x.split(" ")).flatMapValues(lambda x: x).map(lambda x: (int(x[0]),int(x[1]))).partitionBy(128)
