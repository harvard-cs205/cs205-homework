...
# Make page_names
# titles-sorted.txt
page_names = sc.textFile('s3://Harvard-CS205/wikipedia/titles-sorted.txt').zipWithIndex().map(lambda x: (x[1] + 1, x[0])).partitionBy(8).cache()
page_names = sc.textFile('').zipWithIndex().map(lambda x: (x[1] + 1, x[0])).partitionBy(8).cache()
...
page_links = page_links_with_strings.map(lambda line: (line[0], [int(x) for x in line[1].split(' ')])).partitionBy(8).cache()
