...
def connected_components(graph_edges,numPartitions,sc):
...
###############################################
	...
	while rank_list.count()!=0:	
		start_idx=rank_list.values().takeSample(1,1)
		level=0
		graph_init=(start_idx,((level,(start_idx,)),))
		rank_list=rank_list.map(lambda i:graph_init if i[0]==start_idx else i,\
			preservesPartitioning=True)
	...
	#	for level in range(Nmax):
		...
		# previous iterations, so now we need to count the filtered list
			...
			print(graph_size)
			level+=1
	#	once we finish searching with the current start node, we have completely mapped
	#	one connected component. We will remove all of those nodes from the rank list,
	#	and try again
		rank_list=rank_list.filter(lambda (K,V):V==None) 
