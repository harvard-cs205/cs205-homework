...
def shortest_path(index1, index2, links_rdd, numPartitions = 64):
    solutions_found = links_rdd.context.accumulator(0)
    nodes_to_search = links_rdd.filter(lambda x: x[0] == index1).partitionBy(numPartitions)
    path_rdd = nodes_to_search.map(lambda x: (x[0], [x[0]])).partitionBy(numPartitions)
    solution_rdd = path_rdd
    while solutions_found.value == 0:
        join_rdd = links_rdd.join(nodes_to_search).join(path_rdd)
        path_rdd = join_rdd.flatMap(lambda x: add_to_path(x, solutions_found, index1, index2)).partitionBy(numPartitions)
        nodes_to_search = path_rdd.map(lambda x: x[0]).distinct().map(lambda x: (x, [])).partitionBy(numPartitions)
...
if __name__ == '__main__':
    ...
    links_kv = links.map(split_link).partitionBy(partitions).cache()
    harvard_index, bacon_index = title_index('Harvard_University', 'Kevin_Bacon', index_titles)

    print get_names_path(shortest_path(harvard_index, bacon_index, links_kv, partitions), index_titles)
    print get_names_path(shortest_path(bacon_index, harvard_index, links_kv, partitions), index_titles)