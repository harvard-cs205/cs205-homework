                            ...
                            long N)
{
    ...
    for (int ii = get_global_id(0); ii < N; ii += k) { // YOUR CODE HERE
        sum += x[ii]; // YOUR CODE HERE 
    }
    ...
    for (int offset = get_local_size(0) >> 1; offset > 0; offset >>= 1) { // YOUR CODE HERE
        if(local_id < offset) fast[local_id] += fast[local_id + offset];
        barrier(CLK_LOCAL_MEM_FENCE); // YOUR CODE HERE
    }
