                            ...
                            long N)
{
    ...
    size_t j;
    // thread i (i.e., with i = get_global_id(0)) should add x[i],
    // x[i + get_global_size(0)], ... up to N-1, and store in sum.
    for (j=0;i+j*get_global_size(0)<N;j++) { // YOUR CODE HERE
        sum=sum+x[i+j*get_global_size(0)];
    }
    ...
    for (j=get_local_size(0)/2;j>0;j>>=1) { // YOUR CODE HERE
        if(local_id<j)
        {
                          ...
                          long N)
{
    float sum = 0;
    size_t local_id = get_local_id(0);
    int L = ceil((float)N / get_global_size(0));
