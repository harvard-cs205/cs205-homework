                            ...
                            long N)
{
    ...
    for (int idx = global_id; idx < N; idx += global_sz) {
        sum += x[idx];
    }
    ...
    for (uint s = local_sz/2; s > 0; s >>= 1) {
        if (local_id < s) {
            fast[local_id] += fast[local_id + s];
        }
        barrier(CLK_LOCAL_MEM_FENCE);
    }
    if (local_id == 0) partial[group_id] = fast[local_id];
}
                          ...
                          long N)
{
    ...
    for (int idx = k * global_id; idx < k * (global_id + 1) && idx < N; idx++) { 
        sum += x[idx];
    }
