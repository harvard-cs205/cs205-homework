                            ...
                            long N)
{
    ...
    size_t idx = global_id;
    for (idx; idx < N; idx += global_size)
    {
        sum += x[idx];
    }
                          ...
                          long N)
{
    ...
    size_t idx = k * global_id;
    size_t range_bound = k * (global_id + 1);
    for (idx; idx < range_bound; idx++)
    {
        if (idx < N)
            sum += x[idx];
    }
