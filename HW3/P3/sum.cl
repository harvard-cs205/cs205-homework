                            ...
                            long N)
{
    ...
    for (int i = get_global_id(0); i < N ; i += k) { // YOUR CODE HERE
    ...
    for (uint offset = get_local_size(0)/2 ; offset > 0; offset >>= 1) { // YOUR CODE HERE
        if (local_id < offset) {
        fast[local_id] += fast[local_id + offset]; // YOUR CODE HERE
        }
        barrier(CLK_LOCAL_MEM_FENCE);
    }
                          ...
                          long N)
{
    ...
    // Make sure that we do not read past the end of the input vector
    for (int i = global_id * k; (i < ((global_id + 1) * k)) && (i < N); i++) { // YOUR CODE HERE
        sum += x[i]; // YOUR CODE HERE
    }
