...
if __name__ == '__main__':
    ...
    # loop over different programs
    program_files = ['label_regions_partI.cl', \
    'label_regions_partII.cl', \
    'label_regions_partIII.cl', \
    'label_regions_partIV.cl', \
    'label_regions_partV.cl']

    # store rows in this matrix
    data = []

    # iterate over all parts
    for p in xrange(len(program_files)):

        # use for mean
        total_time_agg = 0.0
        time_per_iter_agg = 0.0
        itercount_agg = 0
        
        # iterate every program num_iterations times to get a meaned runtime
        num_iterations = 25
        ...
        for iteration in xrange(num_iterations):
            ...
            # initialize labels
            program = cl.Program(context, open(program_files[p]).read()).build(options='')
            ...
            total_time_agg += total_time
            time_per_iter_agg += total_time / itercount
            itercount_agg += itercount
        print itercount_agg

        # compute mean and store it as row in data matrix
        row = [p+1, itercount_agg / num_iterations, total_time_agg / num_iterations, time_per_iter_agg / num_iterations]
        data.append(row)
    # write csv file with data
    with open('batch_results.csv', 'wb') as csvfile:
        writer = csv.writer(csvfile, dialect='excel')
        writer.writerow(['part', 'iterations', 'total time', 'time per iteration'])
