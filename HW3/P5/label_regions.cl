                 ...
                 const int halo) 
{
    ...
    //PART 4
    int last_fetch, current_fetch;
    if (lx==(get_local_size(0)-1) && ly==(get_local_size(1)-1)){ //using a single thread
        for (int col = halo; col < buf_w-halo; col++){
            for (int row = halo; row < buf_h-halo; row++){
                current_fetch=buffer[row * buf_w + col];
                if (current_fetch<w*h){
                    if (current_fetch==last_fetch){
                        buffer[row * buf_w + col]=last_fetch;
                    }
                    else{ 
                        buffer[row * buf_w + col]=labels[current_fetch];
                        last_fetch=buffer[row * buf_w + col];
                    }
                }
            }
        }
    ...
    //PART 2
    //if (buf_x<buf_w-halo || buf_x>=halo) {
        //if (buf_y<buf_h-halo || buf_y>=halo){ 
            //if (old_label<w*h) {
    ...
    barrier(CLK_LOCAL_MEM_FENCE); //ensure that the local values have finished updating before the minimum neighbor calculation
    ...
    if ((x < w) && (y < h)) {
        ...
        //new_label = old_label;
        int upper = buffer[(buf_y-1) * buf_w + buf_x];
        int left = buffer[buf_y * buf_w + buf_x-1];
        int right = buffer[buf_y * buf_w + buf_x+1];
        int lower = buffer[(buf_y+1) * buf_w + buf_x];
        if (old_label<w*h) {
            new_label=min(old_label, min(upper, min(left, min(right, lower))));
        }
