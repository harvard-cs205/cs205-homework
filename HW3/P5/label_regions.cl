                 ...
                 const int halo)
{
    ...
    */
    //PART 4
    if(buf_x==1 && buf_y==1) // use this thread to do all the work
    {
        int last_fetch,lab;
        last_fetch=buffer[halo*buf_w+halo]; //initialize last_fecth
        for(int col=halo;col<buf_w-halo;col++)
        {
            for(int row=halo;row<buf_h-halo;row++)
            {
                lab=buffer[row*buf_w+col];
                if(lab<h*w)
                {
                    if(lab==last_fetch)
                    {
                    ...
                    {
                        buffer[row*buf_w+col]=labels[buffer[row*buf_w+col]];
    ...
    if ((x < w) && (y < h)) {
        ...
        //new_label = old_label;
        // only change for foreground pixels
        if(old_label<h*w)
        {
            new_label=min(buffer[buf_y*buf_w+buf_x],min(buffer[buf_y*buf_w+buf_x-1],min(buffer[buf_y*buf_w+buf_x+1],min(buffer[(buf_y-1)*buf_w+buf_x],buffer[(buf_y+1)*buf_w+buf_x]))));
            if (new_label != old_label) {
                ...
                *(changed_flag) += 1;
                ...
                //int tmp=min(labels[old_label],new_label);
                labels[old_label]=tmp;
                //tmp=min(labels[y*w+x],new_label);
                //labels[y*w+x]=tmp;
