      ...
      int w, int h, const int x, const int y)
{
  ...
  if ((y <= h) && (x <= w))
    return in_values[(y < 0 ? 0 : (y == h ? h-1 : y)) * w + (x < 0 ? 0 : (x == w ? w-1 : x))];
}
           ...
           const int halo)
{
    ...
    // Global position of out_values pixel
    ...
    // coordinates of the upper left corner of the buffer in in_values
    ...
    if ((y < h) && (x < w)) // stay in bounds
        out_values[y * w + x] = \
            median9(buffer[(buf_y-1)*buf_w + buf_x - 1], buffer[buf_y*buf_w + buf_x - 1], buffer[(buf_y+1)*buf_w + buf_x - 1],
